...
import (
	...
	"github.com/rafaelmvale/fullcycle/go/internal/freight/entity"
	"github.com/rafaelmvale/fullcycle/go/internal/freight/infra/repository"
	"github.com/rafaelmvale/fullcycle/go/internal/freight/usecase"
	"github.com/rafaelmvale/fullcycle/go/pkg/kafka"
)
...
func main() {
	...
	topics := []string{"route"}
	servers := "kubernetes.docker.internal:9094"
	go kafka.Consume(topics, servers, msgChan)

	db, err := sql.Open("mysql", "root:root@tcp(kubernetes.docker.internal:3306)/routes?parseTime=true")
	if err != nil {
		panic(err)
	}
	defer db.Close()
