...
export function NewRoutePage() {
  ...
  async function searchPlaces(event: FormEvent) {
    ...
    const [sourceResponse, destinationResponse] = await Promise.all([
      fetch(`http://localhost:3002/api/places?text=${source}`),
      fetch(`http://localhost:3002/api/places?text=${destination}`),
    ]);
    ...
    const directionsResponse = await fetch(
      `http://localhost:3002/api/directions?originId=${placeSourceId}&destinationId=${placeDestinationId}`
    );
